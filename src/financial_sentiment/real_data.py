...
def load_financial_phrasebank(config: str = "sentences_75agree") -> pd.DataFrame:
    try:
        ...
        raise RuntimeError("Dataset support is not installed. Run: pip install -e .[data]") from exc
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download Financial PhraseBank and export text/label CSV.")
    parser.add_argument("--config", default="sentences_75agree")
    parser.add_argument("--output", default="data/processed/financial_phrasebank.csv")
